...
int main()
{
    ...
    printf("\n%d ",tos);
    while(t<tos){
        printf("\nbus:%s",route[t].bus_name);
        printf("\nstart in %s ",route[t].start_location);
        printf("start at %d:%d",route[t].stime.hour,route[t].stime.min);
        printf("\nstop in %s ",route[t].end_location);
        printf("stop at %d:%d",route[t].etime.hour,route[t].etime.min);
        t++;
...
int match(char *from, char *to, struct time time){
    ...
    for(int t=no_bus-1;t>-1;t--)
    {
        ...
        if((!strcmp(bus[t].start_location,from)&&!strcmp(bus[t].end_location,to))&&((bus->stime.hour>time.hour)||(bus->stime.hour<time.hour+1)))
           {printf("\nmatched ");
           push(bus[t]);
           return 1;
           }
